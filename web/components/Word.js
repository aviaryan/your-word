...
export default class Word extends Component {
	...
	componentDidMount() {
		this.props.contract.methods
			.getNickByAddress(this.props.owner)
			.call()
			.then((nick) => {
				if (nick) {
					this.setState({ nick })
	...
	resolve(status) {
		...
		this.props.contract.methods
			.resolveWord(this.props.id_, status)
			.send({
					...
					{this.props.resolved ? (
						...
						<span>
							{this.props.owner === this.props.account ? (
								<span>
									<a onClick={() => this.resolve(true)} className={styles.btn}>
										✅
									</a>{' '}
									⁄{' '}
									<a onClick={() => this.resolve(false)} className={styles.btn}>
										❌
									</a>
								</span>
							) : (
