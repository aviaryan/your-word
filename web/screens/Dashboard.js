...
export default class Dashboard extends Component {
						...
						.then((res) => {
							wordMap[i] = (
								<Word
									...
									verdict={res[4]}
									id_={i}
									updated={this.updatePage.bind(this)}
	...
	loadNick() {
		this.props.contract.methods
			.getNickByAddress(this.props.account)
			.call()
			.then((nick) => {
				this.setState({ nick })
			})
				...
				<div className={styles.left}>
					<div>
						<p>
							...
							<input
								type='text'
								placeholder='make $5k/mo by December'
								id='text'
						...
						<p>
							...
							/>
							eth against it.
							<span className={styles.button} onClick={this.pledge.bind(this)}>
								DO IT!
							</span>
						</p>
					</div>
						...
						<div>
							{this.state.nickEdit ? (
								<NickEdit
									...
									completed={this.nickEditFalse.bind(this)}
									contract={this.props.contract}
									account={this.props.account}
									showLoading={this.props.showLoading}
								/>
							) : (
								<span onClick={() => this.setState({ nickEdit: true })} className={styles.nick}>
									{this.state.nick}
								</span>
							)}
						</div>
